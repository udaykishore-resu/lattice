...
object DemoGraphsSpec extends ZIOSpecDefault:
  ...
  def spec = suite("DemoGraphs")(
    ...
    test("fulfillment refuses a declined decision") {
      val s = FulfillmentSeed(Applicant("bad-1", 1, 1), Decision("DECLINED", 300, List("x"), None)).toJsonAST.toOption.get
    ...
    test("catalog cross-graph node search sees both graphs") {
      (for
        ...
        all  <- c.list
      yield assertTrue(all.map(_.name).toSet == Set("credit-decision", "fulfillment"), hits.exists(_.node.id == "provision-account")))
