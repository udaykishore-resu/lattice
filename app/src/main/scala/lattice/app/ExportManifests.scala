...
/** Build-time manifest export: `sbt "app/runMain lattice.app.ExportManifests manifests"`. Writes one JSON per graph. CI
  * publishes these so the catalog can be queried with no JVM running.
  */
object ExportManifests extends ZIOAppDefault:
  def run: ZIO[ZIOAppArgs, Throwable, Unit] =
    for
      ...
      ms <- ZIO
        .serviceWithZIO[GraphCatalog](_.list)
