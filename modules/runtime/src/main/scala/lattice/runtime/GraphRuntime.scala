...
/**
 * Interprets a Graph value: levelizes it, runs each level in parallel, applies per-node policies, and
 * returns a single ExecutionRecord. Never fails — failure is a status on the record, so the caller always
 * has something to persist and return.
 */
object GraphRuntime:
      ...
      _       <- result match
                   case Right(NodeOutcome.Produced(v)) =>
                     values.update(_ + (meta.id -> v)) *>
                       records.update(NodeRecord(meta.id.value, meta.kind, NodeStatus.Succeeded, started, ended - started, None) :: _)
                   case Right(NodeOutcome.Skipped) =>
                     values.update(_ + (meta.id -> None)) *>
                       records.update(NodeRecord(meta.id.value, meta.kind, NodeStatus.Skipped, started, ended - started, None) :: _)
                   case Left(err) =>
                     records.update(
